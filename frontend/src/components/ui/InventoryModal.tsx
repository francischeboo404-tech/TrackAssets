...
export const InventoryModal: React.FC<InventoryModalProps> = ({
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      const createdCategoryId = result?.category?.id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim())
      return addToast("error", "Validation Error", "Item name is required");
    if (!formData.sku.trim())
      return addToast("error", "Validation Error", "SKU is required");
    if (!formData.warehouse_id)
      return addToast("error", "Validation Error", "Warehouse selection is required");
    ...
    try {
      const payload = {
        ...
        supplier_item_reference: formData.supplier_item_reference?.trim() || undefined,
        purchase_cost: Number(formData.purchase_cost) || undefined,
        last_purchase_cost: Number(formData.last_purchase_cost) || undefined,
        tax_category: formData.tax_category?.trim() || undefined,
        lead_time_days: Number(formData.lead_time_days) || undefined,
        min_stock_level: Number(formData.min_stock_level) || undefined,
        max_stock_level: Number(formData.max_stock_level) || undefined,
        safety_stock: Number(formData.safety_stock) || undefined,
        opening_stock: Number(formData.opening_stock) || undefined,
      ...
      let msg =
        err.response?.data?.message || "Please verify form data and try again.";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="kg">Milligram (mg)</option>
                <option value="kg">Gram (g)</option>
                <option value="kg">Kilogram (kg)</option>
                <option value="kg">Metric Ton (ton)</option>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const previousSupplier = suppliers.find(
                    (supplier: any) => supplier.id === formData.preferred_supplier_id,
                  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, batch_tracking: e.target.checked })
                }
              />
              <label
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    serial_tracking: e.target.checked,
                  })
                }
              />
              <label
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    expiry_tracking: e.target.checked,
                  })
                }
              />
              <label
