...
export const AssetModal: React.FC<AssetModalProps> = ({
  ...
  const { user } = useAuth();
  const readOnly = asset
    ? !canEditAsset(user?.role)
  ...
  const selectedDepartment = departments?.find(
    (dept: any) => String(dept.id) === String(formData.department_id),
  );

  const allowedAssetTypes: string[] =
    selectedDepartment?.allowed_asset_types?.length > 0
      ? selectedDepartment.allowed_asset_types
      : DEFAULT_ASSET_TYPES;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim())
      return addToast("error", "Validation Error", "Asset name is required");
    ...
    try {
      ...
      if (asset) {
        ...
        addToast(
          "success",
          "Asset Registered",
          `${formData.name} has been added to the portfolio.`,
        );
      }
      ...
      <form id="asset-form" onSubmit={handleSubmit} className="space-y-6">
        {readOnly && (
          <p className="text-sm text-slate-500 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
            View-only mode. Status changes are available from the asset card
            actions.
          </p>
        )}
        <fieldset
            ...
            <div className="space-y-2">
              ...
              {selectedDepartment?.allowed_asset_types?.length > 0 && (
                <p className="text-[11px] text-slate-400 mt-1">
                  Asset types restricted by department:{" "}
                  {selectedDepartment.allowed_asset_types.join(", ")}.
                </p>
              )}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    useful_life: Number(e.target.value),
                  })
                }
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {warehouses?.map((wh: any) => (
                  <option
                    key={wh.id || wh.warehouse_id}
                    value={wh.id || wh.warehouse_id}
                  >
